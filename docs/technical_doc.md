...
```js
/(?:^|\s\(|\s{1,100})(METTRE|TRANSFERER)\s+(LE|LA|LES)?\s*[\p{Graph}]{1,81}\s+(DANS)\s+(202\S)(?:$|\)\s|\s+)/gm;
// Example of a regex generated by the ExpressionMatcher
```

![Regex 101](../res/img/regex.png)
...
- The **Solution Extender** allowed me to really make a great usage of the SQL queries and the multithreading system. The SQL queries were really optimized and allowed me to extend the solutions in a single query.

- Go check the [SQL Query](../big_query.sql) !
